...
player_markers_main_loop_handle = _this spawn {
	...
	while {true} do {
		...
		// show players or players's vehicles
		{
			...
			if((_player distance _x) <= 150) then { 
				if(_show) then {
	...
	//						_text = format["%1 %2", name driver _vehicle, _text];	
							_text = format["%2", name driver _vehicle, _text];
						};							 						
	...
	//								_text = format["%1 %2", _text, name (crew _vehicle select 0)];
									_text = format["%1", _text, name (crew _vehicle select 0)];
									_num = _num - 1;
								};
								if (_num>0) then {
...
//					_markerText setMarkerTextLocal format["%1 [%2]", name _unit, getText(configFile>>"CfgVehicles">>typeOf _x>>"DisplayName")];	
					_markerText setMarkerTextLocal format["[%2]", name _unit, getText(configFile>>"CfgVehicles">>typeOf _x>>"DisplayName")];	
				};
			};
		} forEach allUnitsUav; 			
